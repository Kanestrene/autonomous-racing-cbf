...
try:
    from obstacle_utils import obstacle_angle, obstacle_axes, obstacle_center
except ImportError:
    from .obstacle_utils import obstacle_angle, obstacle_axes, obstacle_center
...
def cbf_rows_for_circle_obstacles(x, y, th, obstacles,
    ...
    Cada obstaculo gera 1 restricao CBF.

    Obstaculos aceites:
    - circulo legado: {"x":..., "y":..., "r":...}
    - elipse: {"x":..., "y":..., "a":..., "b":..., "theta":...}
    a/b sao semi-eixos em metros; theta e opcional em radianos.
    """
    a, b = ellipse_ab
    l = lookahead_l
    ...
    for obs in obstacles:
        ox, oy = obstacle_center(obs)
        obs_a, obs_b = obstacle_axes(obs)
        obs_theta = obstacle_angle(obs)

        # Direcao obstaculo -> ponto lookahead para inflar pela elipse do robo.
        dx_c = px - ox
        dy_c = py - oy
        ...
        obs_a_safe = obs_a + r_robot + margin
        obs_b_safe = obs_b + r_robot + margin

        # Barreira no ponto lookahead, no referencial local da elipse.
        dx = px - ox
        dy = py - oy
        c = np.cos(obs_theta)
        s = np.sin(obs_theta)
        qx = c*dx + s*dy
        qy = -s*dx + c*dy
        h_val = (qx/obs_a_safe)**2 + (qy/obs_b_safe)**2 - 1.0

        grad_local_x = 2.0*qx/(obs_a_safe*obs_a_safe)
        grad_local_y = 2.0*qy/(obs_b_safe*obs_b_safe)
        grad_x = c*grad_local_x - s*grad_local_y
        grad_y = s*grad_local_x + c*grad_local_y

        # p_dot = [ v cos(th) - l w sin(th),
        #           v sin(th) + l w cos(th) ]
        # \dot h = grad_h . p_dot = a_v v + a_w w
        a_v = grad_x*np.cos(th) + grad_y*np.sin(th)
        a_w = l*(-grad_x*np.sin(th) + grad_y*np.cos(th))
