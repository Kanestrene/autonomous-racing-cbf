...
def cbf_clf_qp_filter(
    ...
    if k_heading != 0.0:
        w_ref = float(clf_info["w_ref"])
        epsi = float(clf_info["epsi"])
        w_nom = w_ref #- float(k_heading) * epsi
        w_nom = float(np.clip(w_nom, w_bounds[0], w_bounds[1]))

    clf_info["w_nom_input"] = float(w_nom_input)
    clf_info["w_nom_qp"] = float(w_nom)
    clf_info["k_heading"] = float(k_heading)
