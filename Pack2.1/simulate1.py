...
def make_obstacles(px, py, pyaw, n_obs=5, path_offset_m=DEFAULT_OBSTACLE_PATH_OFFSET_M):
    n_path = len(px)
...
def save_track_only(obstacle_path_offset_m=DEFAULT_OBSTACLE_PATH_OFFSET_M):
    ...
    obstacles = make_obstacles(px, py, pyaw, path_offset_m=obstacle_path_offset_m)
...
def simulate():
    ...
    def show_velocity_plots():
        ...
        print(
            ...
            f"{linear_speed_pdf_path}, {angular_speed_pdf_path} e {delta_pdf_path}"
        )
        plt.show()
    ...
    for k in range(steps):
        ...
        cte = clf_info["ey"]
        w_nom_qp = clf_info.get("w_nom_qp", w_nom)
        delta_controller = omega_to_delta(w_nom_qp, v_nom, L, v_min=0.2)
        ...
        omega_controller_hist.append(w_nom_qp)
