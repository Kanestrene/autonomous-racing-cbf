...
const float steering_deadband = 0.05f;
const bool invert_drive = true;
...
void applyDrive(float v) {
  int pwm_v = velocity_to_pwm(v);

  if (fabs(v) <= velocity_deadband || pwm_v == 0) {
    stopDrive();
    return;
  }
...
void applySteering(float d) {
  int pwm_d = delta_to_pwm(d);

  if (fabs(d) <= steering_deadband || pwm_d == 0) {
    stopSteering();
    return;
  }

  bool positive = d > 0.0f;
  if (invert_steering) {
    positive = !positive;
  }
